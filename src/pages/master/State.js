...
const columns = [
    ...
    },
    {
        ...
        cell: (row) => (
            <ActionComponent 
                slug={row.slug} 
                deleteMessage = "Are you sure want to delete this state?" 
                editUrl = "states" 
                deleteUrl = {BASE_API_URL+"master-state/"}
...
const State = () => {
    ...
    const getData = useCallback(async (page = 1, limit = 10) => {
        ...
        axios.get(`${BASE_API_URL}master-state/states/search/pagination?term=${newSearchTerm}&page=${page}&limit=${limit}`).then((response) => {
    ...
    return(
        <>
            <Container fluid className='section'>
                {isLoading && <Loader/>}
                <PageTitle pageTitle="States" pageUrl="/states/add-state" btnName="Add State" />
