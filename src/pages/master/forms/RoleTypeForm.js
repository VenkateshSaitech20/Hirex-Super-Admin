...
const RoleTypeForm = () => {
    ...
    const getBySlug = useCallback(() => {
        setIsLoading(true);
        axios.get(BASE_API_URL + 'master-roletype/' + slug).then((response) => {
            if (response?.data) {
    ...
    // Get slug
    const getSlug = () => {
        axios.get(BASE_API_URL+'master-roletype/slugs').then((response)=>{
            if(response?.data){
    ...
    const saveData = (data) => {
        if(!duplicateFound){
            setIsLoading(true);
            let slug_name = data.roleType?.toLowerCase().replace(/[^a-zA-Z0-9]+/g, "-");
    ...
    const createRole = (data) => {
        ...
        axios.post(BASE_API_URL+'master-roletype', data).then((response) => {
    ...
    const updateRole = (data) => {
        data.updatedUser = username;
        axios.put(BASE_API_URL+'master-roletype/'+data._id,data).then((response)=>{
            if(response?.data?.result === true) {
                setIsLoading(false);
                reset();
                navigate('/roletype');
    ...
    return(
        <>
            <Container fluid className='section'>
                {isLoading && <Loader />}
                <PageTitle 
                    pageTitle={slug ? 'Edit Role Type' : 'Add Role Type'} 
                    pageUrl="/roletype" 
                    btnName="Back" 
                />
                            ...
                            <Card.Body className="pb-0">
                                <Form autoComplete="off" onSubmit={handleSubmit(saveData)}>
                                    <Form.Group as={Row} className="mb-3">
                                        <Form.Label column="true" md={3}>Role Type</Form.Label>
                                        <Col md={9}>
                                            {slug && 
                                            ...
                                            ></Form.Control>
                                            { errors?.roleType && 
                                                <span className="text-danger">{errors?.roleType && errors.roleType.message}</span>
                                            }
                                            { err?.roleType && 
                                                <span className="text-danger">{err?.roleType}</span>
                                            }
                                            { findDuplicateName && 
                                                <FindDuplicate 
                                    ...
                                    <Row className="mb-3">
                                        <Col md={12} className="text-center">
                                            ...
                                            <HirexButton btntype="cancel" flag="link" linkname="Cancel" redirectTo="/roletype" />
                                        </Col>
                                    </Row>
                                </Form>
                            </Card.Body>
