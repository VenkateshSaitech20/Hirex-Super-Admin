...
const ProgrammingLanguageForm = () => {
    ...
    const getBySlug = useCallback(() => {
        setIsLoading(true);
        axios.get(BASE_API_URL + 'master-programming-language/' + slug).then((response) => {
            if (response?.data) {
    ...
    // Get slug
    const getSlug = () => {
        axios.get(BASE_API_URL+'master-programming-language/slugs').then((response)=>{
            if(response?.data){
    ...
    const saveData = (data) => {
        if(!duplicateFound){
            setIsLoading(true);
            let slug_name = data.langName?.toLowerCase().replace(/[^a-zA-Z0-9]+/g, "-");
    ...
    // Create data
    const createCompanyType = (data) => {
        data.createdUser = username;
        data.updatedUser = username;
        axios.post(BASE_API_URL+'master-programming-language', data).then((response) => {
    ...
    // Update data
    const updateCompanyType = (data) => {
        data.updatedUser = username;
        axios.put(BASE_API_URL+'master-programming-language/'+data._id,data).then((response)=>{
            if(response?.data?.result === true) {
                setIsLoading(false);
                reset();
                navigate('/programming-language');
    ...
    return(
        <>
            <Container fluid className='section'>
                {isLoading && <Loader />}
                <PageTitle 
                    pageTitle={slug ? 'Edit Language' : 'Add Language'} 
                    pageUrl="/programming-language" 
                    btnName="Back" 
                />
                            ...
                            <Card.Body className="pb-0">
                                <Form autoComplete="off" onSubmit={handleSubmit(saveData)}>
                                    <Form.Group as={Row} className="mb-3">
                                        <Form.Label column="true" md={3}>Programming Language</Form.Label>
                                        <Col md={9}>
                                            {slug && 
                                            ...
                                            <Form.Control type="text"
                                                ...
                                                isInvalid = {errors?.langName}
                                            ></Form.Control>
                                            { errors?.langName && 
                                                <span className="text-danger">{errors?.langName && errors.langName.message}</span>
                                            }
                                            { err?.langName && 
                                                <span className="text-danger">{err?.langName}</span>
                                            }
                                            { findDuplicateName && 
                                                <FindDuplicate 
                                    ...
                                    <Row className="mb-3">
                                        <Col md={12} className="text-center">
                                            ...
                                            <HirexButton btntype="cancel" flag="link" linkname="Cancel" redirectTo="/programming-language" />
                                        </Col>
                                    </Row>
                                </Form>
                            </Card.Body>
