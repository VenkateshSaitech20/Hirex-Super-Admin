...
const ProofForm = () => {
    ...
    const getBySlug = useCallback(() => {
        setIsLoading(true);
        axios.get(BASE_API_URL + 'master-proof/' + slug).then((response) => {
            if (response?.data) {
    ...
    // Get slug
    const getSlug = () => {
        axios.get(BASE_API_URL+'master-proof/slugs').then((response)=>{
            if(response?.data){
    ...
    const saveData = (data) => {
        if(!duplicateFound){
            setIsLoading(true);
            let slug_name = data.proofName?.toLowerCase().replace(/[^a-zA-Z0-9]+/g, "-");
    ...
    const createCompanyType = (data) => {
        ...
        axios.post(BASE_API_URL+'master-proof', data).then((response) => {
    ...
    const updateCompanyType = (data) => {
        data.updatedUser = username;
        axios.put(BASE_API_URL+'master-proof/'+data._id,data).then((response)=>{
            if(response?.data?.result === true) {
                setIsLoading(false);
                reset();
                navigate('/proof');
    ...
    return(
        <>
            <Container fluid className='section'>
                {isLoading && <Loader />}
                <PageTitle 
                    pageTitle={slug ? 'Edit Proof' : 'Add Proof'} 
                    pageUrl="/proof" 
                    btnName="Back" 
                />
                            ...
                            <Card.Body className="pb-0">
                                <Form autoComplete="off" onSubmit={handleSubmit(saveData)}>
                                    <Form.Group as={Row} className="mb-3">
                                        <Form.Label column="true" md={3}>Proof</Form.Label>
                                        <Col md={9}>
                                            {slug && 
                                            ...
                                            <Form.Control type="text"
                                                ...
                                                isInvalid = {errors?.proofName}
                                            ></Form.Control>
                                            { errors?.proofName && 
                                                <span className="text-danger">{errors?.proofName && errors.proofName.message}</span>
                                            }
                                            { err?.proofName && 
                                                <span className="text-danger">{err?.proofName}</span>
                                            }
                                            { findDuplicateName && 
                                                <FindDuplicate 
                                    ...
                                    <Row className="mb-3">
                                        <Col md={12} className="text-center">
                                            ...
                                            <HirexButton btntype="cancel" flag="link" linkname="Cancel" redirectTo="/proof" />
                                        </Col>
                                    </Row>
                                </Form>
                            </Card.Body>
