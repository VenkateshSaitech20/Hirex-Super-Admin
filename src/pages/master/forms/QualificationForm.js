...
const QualificationForm = () => {
    ...
    const getBySlug = useCallback(() => {
        setIsLoading(true);
        axios.get(BASE_API_URL + 'master-qualification/' + slug).then((response) => {
            if (response?.data) {
    ...
    // Get slug
    const getSlug = () => {
        axios.get(BASE_API_URL+'master-qualification/slugs').then((response)=>{
            if(response?.data){
    ...
    const saveData = (data) => {
        if(!duplicateFound){
            setIsLoading(true);
            let slug_name = data.qualName?.toLowerCase().replace(/[^a-zA-Z0-9]+/g, "-");
    ...
    const createCompanyType = (data) => {
        ...
        axios.post(BASE_API_URL+'master-qualification', data).then((response) => {
    ...
    const updateCompanyType = (data) => {
        data.updatedUser = username;
        axios.put(BASE_API_URL+'master-qualification/'+data._id,data).then((response)=>{
            if(response?.data?.result === true) {
                setIsLoading(false);
                reset();
                navigate('/qualification');
    ...
    return(
        <>
            <Container fluid className='section'>
                {isLoading && <Loader />}
                <PageTitle 
                    pageTitle={slug ? 'Edit Qualification' : 'Add Qualification'} 
                    pageUrl="/qualification" 
                    btnName="Back" 
                />
                            ...
                            <Card.Body className="pb-0">
                                <Form autoComplete="off" onSubmit={handleSubmit(saveData)}>
                                    <Form.Group as={Row} className="mb-3">
                                        <Form.Label column="true" md={3}>Qualification</Form.Label>
                                        <Col md={9}>
                                            {slug && 
                                            ...
                                            <Form.Control type="text"
                                                ...
                                                isInvalid = {errors?.qualName}
                                            ></Form.Control>
                                            { errors?.qualName && 
                                                <span className="text-danger">{errors?.qualName && errors.qualName.message}</span>
                                            }
                                            { err?.qualName && 
                                                <span className="text-danger">{err?.qualName}</span>
                                            }
                                            { findDuplicateName && 
                                                <FindDuplicate 
                                    ...
                                    <Row className="mb-3">
                                        <Col md={12} className="text-center">
                                            ...
                                            <HirexButton btntype="cancel" flag="link" linkname="Cancel" redirectTo="/qualification" />
                                        </Col>
                                    </Row>
                                </Form>
                            </Card.Body>
