...
const columns = [
    ...
    },
    {
      ...
      sortable: true,
      cell: (row) => <BadgeComponent status={row.top} />
    },
    {
      name: "Status",
    ...
    },
    {
        ...
        cell: (row) => (
            <ActionComponent 
                slug={row.slug} 
                deleteMessage = "Are you sure want to delete this category?" 
                editUrl = "job-category" 
                deleteUrl = {BASE_API_URL+"master-job-category/"}
...
let jobCategoryUpdateData;

const updateData = () => {
    jobCategoryUpdateData.getData();
}
...
const JobCategory = () => {
    ...
    const getData = useCallback(async (page = 1, limit = 10) => {
        ...
        axios.get(`${BASE_API_URL}master-job-category/categories/search/pagination?term=${newSearchTerm}&page=${page}&limit=${limit}`).then((response) => {
    ...
    return(
        <>
            <Container fluid className='section'>
                {isLoading && <Loader/>}
                <PageTitle pageTitle="Job Category" pageUrl="/job-category/add-job-category" btnName="Add Category" />
