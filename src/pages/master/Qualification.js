...
const columns = [
    ...
    },
    {
        ...
        cell: (row) => (
            <ActionComponent 
                slug={row.slug} 
                deleteMessage = "Are you sure want to delete this qualification?" 
                editUrl = "qualification" 
                deleteUrl = {BASE_API_URL+"master-qualification/"}
...
let qualificationUpdateData;

const updateData = () => {
    qualificationUpdateData.getData();
}
...
const Qualification = () => {
    ...
    const getData = useCallback(async (page = 1, limit = 10) => {
        ...
        axios.get(`${BASE_API_URL}master-qualification/search/pagination?term=${newSearchTerm}&page=${page}&limit=${limit}`).then((response) => {
    ...
    return(
        <>
            <Container fluid className='section'>
                {isLoading && <Loader/>}
                <PageTitle pageTitle="Qualification" pageUrl="/qualification/add-qualification" btnName="Add Qualification" />
