...
const columns = [
    ...
    },
    {
        ...
        cell: (row) => (
            <ActionComponent 
                slug={row.slug} 
                deleteMessage = "Are you sure want to delete this title?" 
                editUrl = "jobtitle" 
                deleteUrl = {BASE_API_URL+"master-jobtitle/"}
...
let jobTitleUpdateData;

const updateData = () => {
    jobTitleUpdateData.getData();
}
...
const JobTitle = () => {
    ...
    const getData = useCallback(async (page = 1, limit = 10) => {
        ...
        axios.get(`${BASE_API_URL}master-jobtitle/search/pagination?term=${newSearchTerm}&page=${page}&limit=${limit}`).then((response) => {
    ...
    return(
        <>
            <Container fluid className='section'>
                {isLoading && <Loader/>}
                <PageTitle pageTitle="Job Title" pageUrl="/jobtitle/add-jobtitle" btnName="Add Job Title" />
