...
const GradeForm = () => {
    ...
    const getBySlug = useCallback(() => {
        setIsLoading(true);
        axios.get(BASE_API_URL + 'grade/' + slug).then((response) => {
            if (response?.data) {
    ...
    // Get slug
    const getSlug = () => {
        axios.get(BASE_API_URL+'grade/slugs').then((response)=>{
            if(response?.data){
    ...
    const saveData = (data) => {
        if(!duplicateFound){
            setIsLoading(true);
            let slug_name = data.gradeName?.toLowerCase().replace(/[^a-zA-Z0-9]+/g, "-");
    ...
    const createGrade = (data) => {
        ...
        axios.post(BASE_API_URL+'grade', data).then((response) => {
    ...
    const updateGrade = (data) => {
        data.updatedUser = username;
        axios.put(BASE_API_URL+'grade/'+data._id,data).then((response)=>{
            if(response?.data?.result === true) {
                setIsLoading(false);
                reset();
                navigate('/grade');
    ...
    return(
        <>
            <Container fluid className='section'>
                {isLoading && <Loader />}
                <PageTitle 
                    pageTitle={slug ? 'Edit Grade' : 'Add Grade'} 
                    pageUrl="/grade" 
                    btnName="Back" 
                />
                            ...
                            <Card.Body className="pb-0">
                                <Form autoComplete="off" onSubmit={handleSubmit(saveData)}>
                                    <Form.Group as={Row} className="mb-3">
                                        <Form.Label column="true" md={3}>Grade</Form.Label>
                                        <Col md={9}>
                                            {slug && 
                                            ...
                                            ></Form.Control>
                                            { errors?.gradeName && 
                                                <span className="text-danger">{errors?.gradeName && errors.gradeName.message}</span>
                                            }
                                            { err?.gradeName && <span className="text-danger">{err?.gradeName}</span> }
                                            { findDuplicateName && 
                                                <FindDuplicate 
                                    ...
                                    <Row className="mb-3">
                                        <Col md={12} className="text-center">
                                            ...
                                            <HirexButton btntype="cancel" flag="link" linkname="Cancel" redirectTo="/grade" />
                                        </Col>
                                    </Row>
                                </Form>
                            </Card.Body>
